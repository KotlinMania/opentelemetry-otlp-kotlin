...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `OpentelemetryOtlp.swiftmodule/`
...
//      `import OpentelemetryOtlp` above would fail at compile time.
//
//   2. The static archive `libOpentelemetryOtlp.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "OpentelemetryOtlp" }` and
//      `flattenPackage = "io.github.kotlinmania.opentelemetryotlp"` configuration in
...
//      via the `OpentelemetryOtlpLibrary` product.
//
...
final class OpentelemetryOtlpExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "OpentelemetryOtlp swift module imported cleanly")
    }
}
