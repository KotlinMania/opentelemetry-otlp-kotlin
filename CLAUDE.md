# Claude Code Project Instructions — opentelemetry-otlp-kotlin

## Project Overview

This is **opentelemetry-otlp-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`opentelemetry-otlp`](https://crates.io/crates/opentelemetry-otlp).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/opentelemetry-otlp/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/opentelemetry-otlp>
